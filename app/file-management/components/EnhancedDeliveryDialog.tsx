    ...
    const fetchCompanyPeople = async () => {
        ...
        try {
            const { data: individuals, error } = await supabase
                ...
                .select('id, full_name, role, on_payroll, employment_history')
                .contains('employment_history', [{ company_id: companyId }]);
            ...
            if (individuals) {
                // Filter employees: role = 'employee' or empty + on_payroll = 'yes'
                const employeeList = individuals.filter(person => {
                    const hasEmployeeRole = !person.role || person.role === '' || person.role.toLowerCase() === 'employee';
                    const isOnPayroll = person.on_payroll?.toLowerCase() === 'yes';
                    return hasEmployeeRole && isOnPayroll;
                });

                // Filter directors: role = 'director'
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-3">
                                            <Input
                                                ...
                                                placeholder="Enter person's name or select from below"
                                                            ...
                                                            <div className="space-y-1 max-h-32 overflow-y-auto">
                                                                {employees.map((employee) => (
                                                                    <Button
                                                                        ...
                                                                        onClick={() => setFormData(prev => ({ ...prev, broughtBy: employee.full_name }))}
                                                                    >
                                                            ...
                                                            <div className="space-y-1 max-h-32 overflow-y-auto">
                                                                {directors.map((director) => (
                                                                    <Button
                                                                        ...
                                                                        size="sm"
                                                                        className="w-full justify-start text-left h-auto p-2 text-xs"
                                                                        onClick={() => setFormData(prev => ({ ...prev, broughtBy: director.full_name }))}
                                                                    >
                                                                        <UserCheck className="h-3 w-3 mr-2 text-purple-600" />
                                                                        {director.full_name}
                                                                    </Button>
                                                                ))}
                                                            </div>
                                                        ) : (
                                                            <div className="text-xs text-gray-500">No directors found</div>
                                                        )}
                                                    </CardContent>
                                                </Card>
                                            </div>
