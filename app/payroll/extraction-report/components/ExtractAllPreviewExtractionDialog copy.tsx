...
export function ExtractAllPreviewExtractionDialog({
    ...
    const handleExtractionUpdate = (field: keyof Extraction, value: string) => {
        setLocalDocs(docs =>
            docs.map((doc, i) =>
                i === 0 ? {
                    ...doc,
                    documents: doc.documents.map((d, j) =>
                        j === activeDoc
                            ? {
                                ...d,
                                extractions: {
    ...
    const handleSave = async (companyName: string, companyDocs: Document[]) => {
        ...
        try {
            ...
            onSave(localDocs[0].recordId, updatedExtractions);
    ...
    const renderExtractionField = (field: keyof Extraction, label: string, placeholder: string) => (
        <div className="space-y-1">
            ...
            <Input
                id={field}
                value={localDocs[0].documents[activeDoc].extractions[field] || ''}
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-7xl max-h-[90vh]">
                ...
                <ScrollArea className="h-[calc(90vh-100px)]">
                    {documentsByCompany.map(({ recordId, companyName, documents }) => (
                        <div
                            key={recordId}
                                ...
                                <div className="p-4 border-t">
                                    <div className="grid grid-cols-7 gap-6">
                                        ...
                                        <div className="col-span-3 border rounded bg-white overflow-hidden">
                                            {documents[activeDoc]?.url && (
                                                <iframe
                                                    src={documents[activeDoc].url}
