...
export function ExtractAllDialog({
    ...
    const handleExtractAll = async () => {
        try {
            ...
            // Handle document uploads for each record
            for (const record of records) {
                // Assuming documentType is predefined for each record
                const documentType = 'PAYMENT RECEIPTS';
                const file = new File([], record.company.company_name); // Replace with actual file
                await handleDocumentUpload(record.id, file, documentType);
            }
        ...
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-7xl max-h-[80vh] overflow-hidden flex flex-col">
                    ...
                    <Table>
                        <TableHeader className="sticky top-0 z-10">
                            <TableRow className="hover:bg-transparent">
                                <TableHead rowSpan={2} className="border-r border-b border-gray-200 bg-blue-600 text-white font-semibold min-w-[200px] whitespace-nowrap">
                                    Company
                                </TableHead>
                                {TAX_TYPES.map(tax => (
                                    <TableHead key={tax.id} colSpan={4} className={`text-center text-white font-semibold ${tax.color} border-r border-b`}>
                                        {tax.label}
                                    </TableHead>
                                ))}
                                <TableHead rowSpan={2} className="border-r border-b border-gray-200 bg-blue-600 text-white font-semibold">
                                    Actions
                                </TableHead>
                            </TableRow>
                            <TableRow className="hover:bg-transparent">
                        ...
                        <TableBody>
                            {records.map((record, index) => {
                                return (
                                    <TableRow key={record.id} className={`${index % 2 === 0 ? 'bg-blue-50 hover:bg-blue-100' : 'bg-white hover:bg-gray-50'} [&>td]:border-r [&>td]:border-gray-200 last:[&>td]:border-r-0`}> 
                                        <TableCell className="font-medium truncate max-w-[200px]" title={record.company.company_name}>{record.company.company_name}</TableCell>
                                        {TAX_TYPES.map(tax => {
                                            ...
                                            return (
                                                <>
                                                    <TableCell className="text-right font-mono">{extractedData?.amount || '-'}</TableCell>
                                                    <TableCell className="text-center">{extractedData?.payment_mode || '-'}</TableCell>
                                                    <TableCell className="text-center whitespace-nowrap">{extractedData?.payment_date || '-'}</TableCell>
                                                    <TableCell className="text-center">
                                                        <div className="flex justify-center">{result ? getStatusIcon(result.status) : '-'}</div>
                                                        {result?.error && <span className="text-xs text-red-500 mt-1">{result.error}</span>}
                                                    </TableCell>
                                                </>
                                            );
                                        })}
                ...
                <div className="mt-4 flex justify-end gap-2">
                    ...
                    <Button onClick={handleExtractAll} disabled={processing} className="bg-blue-600 hover:bg-blue-700">
                        {processing ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Processing...</> : 'Extract All'}
                    </Button>
                </div>
            </DialogContent>
        </Dialog>
