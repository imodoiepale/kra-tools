    ...
    import { Loader2, Save, RotateCw, Check, ChevronLeft, ChevronRight, AlertTriangle, CheckCircle } from 'lucide-react'
    ...
    const handleUseForStatement = (balance) => {
        // Update the main form with this month's data
        setOpeningBalance(balance.opening_balance.toString());
        setClosingBalance(balance.closing_balance.toString());

        // Set the selected month/year
        setSelectedMonth(balance.month);
        setSelectedYear(balance.year);

        // Navigate to opening page
        handlePageChange(balance.statement_page);
        ...
        toast({
            ...
            description: `Using balance data from ${format(new Date(balance.year, balance.month - 1, 1), 'MMMM yyyy')}`,
        });
    };
    ...
    export function BankExtractionDialog({
        ...
        const handleExtract = async () => {
            ...
            try {
                setExtracting(true);
                setExtractionProgress('Starting extraction process...');
                ...
                const extractionResult = await performBankStatementExtraction(
                    ...
                    (progressMessage) => {
                        ...
                        setExtractionProgress(progressMessage);
                    }
                );
                ...
                <DialogContent className="max-w-7xl max-h-[90vh] flex flex-col">
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
                        ...
                        <TabsContent value="monthly" className="flex-1 overflow-auto">
                            <div className="p-4 bg-muted rounded-md mb-4 flex justify-between items-center">
                                <div>
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {monthlyBalances.map((balance, index) => {
                                                // Highlight the row that matches the current statement month/year
                                                            ...
                                                            <div className="flex gap-2">
                                                                ...
                                                                {isCurrentPeriod && (
                                                                    <Button
                                                                        variant="ghost"
                                                                        size="sm"
                                                                        onClick={() => handleUseForStatement(balance)}
                                                                        title="Use this balance for the main statement data"
                                                                    >
                    ...
                    {extracting && (
                        <div className="p-4 bg-blue-50 rounded-md mt-2">
                            <div className="flex items-center">
                                ...
                                <p className="text-sm text-blue-700">{extractionProgress || "Extracting data..."}</p>
                            </div>
                        </div>
                    )}
