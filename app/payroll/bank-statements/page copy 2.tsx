...
import { Loader2, UploadCloud, Edit, Eye, CheckCircle, AlertTriangle, MoreHorizontal, Trash, Lock } from 'lucide-react'
...
export function BankReconciliationTable({
    ...
    useEffect(() => {
        console.log('BankReconciliationTable state:', {
            ...
            hasPassword: activeBank?.acc_password ? 'Yes' : 'No'
        });
    ...
    const handleOpenDialog = (dialogType: string, bank: Bank) => {
        ...
        // Log enhanced bank info including password status
        console.log('Opening dialog for bank:', {
            bankName: bank.bank_name,
            companyName: bank.company_name,
            hasStoredPassword: !!bank.acc_password,
            dialogType,
            hasExistingStatement: !!currentStatementForPeriod
        });
    ...
    const getValidationStatusIcon = (statement: BankStatement | null) => {
        ...
        return hasPassword ? <Lock className="h-3 w-3 text-blue-500" /> : null;
    };
    ...
    const finalFilteredData = useMemo(() => {
        ...
        return companiesWithBanks.map(company => ({
            ...
            banks: company.banks.filter(bank => {
                ...
                if (!selectedStatementStatuses || selectedStatementStatuses.length === 0 ||
                    selectedStatementStatuses.some(f => f.key === 'all')) {
                    return true;
                }
    ...
    return (
        <div className="space-y-4">
                ...
                <Table aria-label="Bank Reconciliation" className="border border-gray-200 text-sm">
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={11} className="text-center py-4">
                                    No banks found for the selected filters.
                                </TableCell>
                            </TableRow>
                        ) : (
                            finalFilteredData.map((company, companyIndex) => (
                                company.banks.length === 0 ? (
                                    <TableRow key={`company-${company.id}`} className="bg-gray-50 hover:bg-gray-100">
                                        ...
                                        <TableCell colSpan={9} className="text-center text-red-500 font-bold p-1">
                                            No banks configured
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                    company.banks.map((bank, bankIndex) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                        ...
                                                        {bank.acc_password && (
                                                            <TooltipProvider>
                                                                <Tooltip>
                                                                    ...
                                                                    <TooltipContent>
                                                                        Password stored for auto-unlock
                                                                    </TooltipContent>
                                                                </Tooltip>
                                                            </TooltipProvider>
                                                        )}
                                                ...
                                                <TableCell className="border-r border-gray-200 p-1">
                                                    {statement ? (
                                                        <Button
                                                            variant="outline"
                                                            size="sm"
                                                            className={`relative flex gap-1.5 items-center px-3 py-1.5 ${statement.statement_document?.statement_pdf ?
                                                                    'border-green-300 bg-green-50/50 hover:bg-green-100/60' :
                                                        ...
                                                        >
                                                            ...
                                                            {bank.acc_password && (
                                                                <TooltipProvider>
                                                                    <Tooltip>
                                                                        <TooltipTrigger>
                                                                            <Lock className="h-3 w-3 text-blue-500 ml-1" />
                                                                        </TooltipTrigger>
                                                                        <TooltipContent>
                                                                            Auto-unlock available
                                                                        </TooltipContent>
                                                                    </Tooltip>
                                                                </TooltipProvider>
                                                            )}
                                                ...
                                                <TableCell className={`border-r border-gray-200 text-right p-1 ${difference !== null ?
                                                        (Math.abs(difference) > 0.01 ? 'text-red-500 font-bold' : 'text-green-500 font-bold') :
                                                        ''
                                                    }`}>
                                                ...
                                                <TableCell className="border-r border-gray-200 p-1 truncate">
                                                    <TooltipProvider>
                                                        <Tooltip>
                                                            <TooltipTrigger className="truncate">
                                                                {statement?.status?.status || 'No Statement'}
                                                            </TooltipTrigger>
                                                            <TooltipContent>
                                                                {statement?.status?.status || 'No Statement'}
                                                            </TooltipContent>
                                                ...
                                                <TableCell className="p-1">
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end" className="w-44">
                                                            ...
                                                            >
                                                                <UploadCloud className="h-3.5 w-3.5" />
                                                                {statement ? 'Replace' : 'Upload'}
                                                                {bank.acc_password && (
                                                                    <Lock className="h-3 w-3 text-blue-500 ml-auto" />
                                                                )}
                                                            </DropdownMenuItem>
                                                            <DropdownMenuItem
                                                                onClick={() => handleOpenDialog('qb', bank)}
                                                            ...
                                                            <DropdownMenuItem
                                                                onClick={() => handleOpenDialog('delete', bank)}
                                                                disabled={!statement}
            ...
            {/* Enhanced Upload Dialog with auto password detection */}
            {uploadDialogOpen && activeBank && (
                <BankStatementUploadDialog
                    isOpen={uploadDialogOpen}
                    onClose={() => setUploadDialogOpen(false)}
                    onStatementUploaded={(statement) => {
                        console.log('Statement uploaded with enhanced features:', statement);
                    ...
                    onOpenExtractionDialog={(statement) => {
                        console.log('Auto-opening extraction dialog for enhanced upload:', statement);
                        setActiveStatement(statement);
                        setExtractionDialogOpen(true);
                    }}
                />
            ...
            <AlertDialog open={deleteConfirmationOpen} onOpenChange={setDeleteConfirmationOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will permanently delete the statement and its associated files. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
