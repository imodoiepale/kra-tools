...
export default function WinguAppsExtractionReports() {
    ...
    const latestReports = React.useMemo(() => {
        ...
        // Create a map to store one report per company, prioritizing the latest date
        const companyMap = new Map();

        reports.forEach(report => {
            const existingReport = companyMap.get(report.CompanyID);
            if (!existingReport ||
                (report.Year === latestYear && report.Month === latestMonth)) {
                companyMap.set(report.CompanyID, report);
            }
        });
    ...
    const renderTableRow = (report: ReportRecord, index: number) => (
        ...
        >
            <TableCell className="border-r border-gray-200 text-[11px] font-medium text-gray-600">
                {index + 1}
            </TableCell>
