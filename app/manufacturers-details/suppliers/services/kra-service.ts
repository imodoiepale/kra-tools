...
export const kraService = {
    fetchManufacturerDetails: async (kraPins: string[]): Promise<KraResponse[]> => {
    ...
    startManufacturerDetailsCheck: async (
        kraPins: string[]
    ): Promise<{ results: KraResponse[], summary: { successful: number; failed: number } }> => {
        try {
            // First fetch manufacturer details for all PINs
            const results = await kraService.fetchManufacturerDetails(kraPins);
