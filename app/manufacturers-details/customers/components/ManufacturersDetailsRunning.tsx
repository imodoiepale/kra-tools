...
export function ManufacturersDetailsRunning({ 
    ...
    useEffect(() => {
        const fetchProgress = async () => {
            // Get initial count of customers to process
            const { data: customerData } = await supabase
                .from('acc_portal_kra_customers')
                .select('pin_no')

            setTotalCompanies(customerData?.length || 0)
    ...
    return (
        <div className="space-y-4">
            <h3 className="text-lg font-medium">Customers Details Check in Progress</h3>
            <Progress value={percentComplete} className="w-full" />
            <p className="text-sm text-gray-500">
                {progress} out of {totalCompanies} customers checked ({percentComplete.toFixed(1)}%)
            </p>
            ...
            <div className="overflow-x-auto">
                <div className="overflow-y-auto max-h-[calc(100vh-300px)]">
                    <Table>
                        <TableCaption>Customers Details Check Results</TableCaption>
                        <TableHeader>
                            <TableRow>
